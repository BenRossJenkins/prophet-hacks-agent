...
def manifold_prior(event: dict) -> tuple[float, str] | None:
    ...
    Single-pass search using a rule-based cleaned query from the Kalshi
    title. Returns (p_yes, rationale) on success, or None when no usable
    direct-query match is found — caller then falls through to LLM
    ensemble. The earlier LLM-rephrased fallback was removed 2026-05-20
    after live evidence showed it produced semantically-unrelated matches
    on ~all observed cases (the cleaned-query matches were reliable; the
    rephrased ones were not, see audit memory).
    """
